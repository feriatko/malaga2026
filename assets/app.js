...
function renderVisited(){
  ...
  const total=$$('[data-visited]').length, count=visited.size, pct=Math.round(count/total*100);
  $('#progressValue').textContent=pct+'%'; $('#progressRing').style.setProperty('--p',(pct*3.6)+'deg');
  $('#progressText').textContent=count?`Navštívené ${count} z ${total} miest.`:'Zatiaľ nemáte označené žiadne miesto.';
}
...
$('#resetVisited').addEventListener('click',()=>{if(confirm('Vynulovať všetky navštívené miesta?')){visited.clear();saveVisited()}});
renderVisited();
...
themeBtn.addEventListener('click',()=>{document.body.classList.toggle('darkmode');localStorage.setItem('holiday-theme',document.body.classList.contains('darkmode')?'dark':'light')});
